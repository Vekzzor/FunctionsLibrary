...
{
private:
	template<typename T> int partition(T theArray[], int start, int end);
	template<typename T> int partition(std::vector<T> &vec, int start, int end);
	template<typename T> void swapThem(T &item1, T &item2);
public:
...
int Sort::partition(std::vector<T> &vec, int start, int end)
{
	...
	for (int i = (start + 1); i <= end; i++)
	{
		...
		{
			swapThem(vec.at(pivotPosition + 1), vec.at(i));
			swapThem(vec.at(pivotPosition), vec.at(pivotPosition + 1));
			pivotPosition++;
		}
	}
	return pivotPosition;
